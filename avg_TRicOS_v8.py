...
def baselinefit_cst(df):
    base=df.absor[df.wl.between(675,700)].mean()
    # print(base)
    corrected=df.copy()
...
scaled_avg_spec=rescale_raw(avg_spec,smoothed_spec, 279,281)
scaled_avg_ground=rescale_raw(avg_ground,smoothed_ground, 279,281)

scaled_smoothed_spec=rescale_corrected(smoothed_spec,279,281)
scaled_smoothed_ground=rescale_corrected(smoothed_ground,279,281)


#%%% plot averaged non scaled raw spectra
...
ax.set_title('averaged absorbance in crystallo absorbance spectra (non scaled)', fontsize=10, fontweight='bold')  #This sets the title of the plot
ax.set_xlim([260,700]) 
...
#%%% plot averaged smoothed spectra
...
ax.set_title('averaged smoothed absorbance in crystallo absorbance spectra (unscaled)', fontsize=10, fontweight='bold')  #This sets the title of the plot
ax.set_xlim([260,700]) 
ax.set_ylim([globmin,globmax])
ax.tick_params(labelsize=10)
ax.yaxis.set_ticks(np.arange(round(globmin,1), round(globmax+0.1,1), 0.1))  #This modulates the frequency of the x label (1, 50 ,100 ect)
...
# # plt.show()
# figfilename = "averaged_smoothed_spec.pdf"             #Filename, for the PDF output, don't forget to close the last one before executing again, otherwise python can't write over it
# plt.savefig(figfilename, dpi=300, transparent=True,bbox_inches='tight') 
# figfilename = "averaged_smoothed_spec.png"             #Filename, for the PDF output, don't forget to close the last one before executing again, otherwise python can't write over it
# plt.savefig(figfilename, dpi=300, transparent=True,bbox_inches='tight') 
# figfilename = "averaged_smoothed_spec.svg"             #Filename, for the PDF output, don't forget to close the last one before executing again, otherwise python can't write over it
# plt.savefig(figfilename, dpi=300, transparent=True,bbox_inches='tight') 
# plt.close()
